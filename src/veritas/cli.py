...
def verify(
    ...
    verbose: bool = typer.Option(
        False, "--verbose", "-v", help="Show full content of verified logs"
    ),
):
    """
    ...
    console.print(
        Panel(
            f"Verifying Session Proof: [bold cyan]{proof_file}[/bold cyan]", title="Veritas Auditor"
        )
    )
    ...
    for i, detail in enumerate(details):
        status = (
            "[green]PASS[/green]"
            if "Verified" in detail or "verified" in detail
            else "[yellow]INFO[/yellow]"
        )
        if "Warning" in detail:
            status = "[yellow]WARN[/yellow]"
        if "FAILURE" in detail or "TAMPER" in detail:
            status = "[red]FAIL[/red]"
