...
class ERC721Capability(VeritasCapability):
        ...
        self.tools.append(VeritasTool(
            ...
            parameters={
                ...
                "properties": {
                    ...
                    "address": {"type": "string", "description": "Optional address to check (defaults to agent)"}
                },
    ...
    def get_balance(self, contract_address: str, address: Optional[str] = None) -> Dict[str, Any]:
        ...
        contract = self.agent.w3.eth.contract(address=self.agent.w3.to_checksum_address(contract_address), abi=ERC721_ABI)
...
class BasenameCapability(VeritasCapability):
    ...
    def register(self, basename: str, amount: str = "0.002") -> Dict[str, Any]:
        ...
        registrar_addr = BASENAMES_REGISTRAR_CONTROLLER_ADDRESS_MAINNET if is_mainnet else BASENAMES_REGISTRAR_CONTROLLER_ADDRESS_TESTNET
        ...
        registrar_contract = w3.eth.contract(address=w3.to_checksum_address(registrar_addr), abi=REGISTRAR_ABI)
        
        name_hash = w3.ens.namehash(full_name)
        address_data = resolver_contract.encode_abi("setAddr", args=[name_hash, self.agent.account.address])
