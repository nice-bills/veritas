...
class VeritasVerifier:
    ...
    def verify_session(proof_data: Dict[str, Any]) -> Tuple[bool, str, List[str]]:
        ...
        for i, log_entry in enumerate(logs):
            ...
            if claimed_leaves and i < len(claimed_leaves):
                ...
                if calculated_leaf != expected_leaf:
                    details.append(
                        f"[red]TAMPER DETECTED[/red] in Log #{i} ({log_entry.get('tool_name')})"
                    )
                    ...
                    details.append(
                        f"[green]Verified[/green] Log #{i}: {log_entry.get('tool_name')}"
                    )
        ...
        if calculated_root != claimed_root:
            details.append("[red]CRITICAL FAILURE:[/red] Merkle Root mismatch!")
        ...
        for i, log in enumerate(logs):
            ...
            if basis_id:
                if basis_id not in event_ids:
                    details.append(
                        f"[red]Broken link[/red] at log [{i}]: basis_id {basis_id} not found in session"
                    )
                    valid_chain = False
                else:
                ...
                details.append(
                    f"[yellow]Warning:[/yellow] Action {log['tool_name']} has no linked observation"
                )

        is_valid = valid_root and valid_chain
        message = (
            "Session integrity verified" if is_valid else "Session integrity verification FAILED"
        )
